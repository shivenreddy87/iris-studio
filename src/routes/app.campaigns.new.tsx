  ...
  const steps = [
    {
      ...
      content: (
        <input
          ...
          className="w-full rounded-2xl border border-midnight/10 bg-canvas px-5 py-4 text-lg placeholder:text-midnight/40 focus:border-violet focus:outline-none focus:ring-4 focus:ring-violet/10"
        />
      ),
    ...
    },
    {
      ...
      content: (
        <div>
          <div className="mb-2 font-mono text-xs uppercase tracking-widest text-midnight/40">INR (₹)</div>
          <input
            type="number"
            value={budget}
            onChange={(e) => setBudget(e.target.value)}
            className="w-full rounded-2xl border border-midnight/10 bg-canvas px-5 py-4 text-lg focus:border-violet focus:outline-none focus:ring-4 focus:ring-violet/10"
          />
        </div>
      ),
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 lg:px-8">
      ...
      <div className="mb-8 flex gap-1">
        {steps.map((_, i) => (
          <div
            key={i}
            className={`h-1 flex-1 rounded-full ${i <= step ? "bg-violet" : "bg-midnight/10"}`}
          />
        ))}
      </div>

      <div className="rounded-3xl border border-midnight/5 bg-white p-8 shadow-sm">
        <label className="mb-3 block font-display text-lg font-bold text-midnight">
          {current.label}
        </label>
        {current.content}
        ...
        <div className="mt-6 flex items-center justify-between">
          <button
            ...
            disabled={step === 0}
            className="text-sm font-semibold text-midnight/50 hover:text-midnight disabled:opacity-40"
          >
            Back
          </button>
          {step < steps.length - 1 ? (
