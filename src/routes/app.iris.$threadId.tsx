...
import { getIrisMessages } from "@/lib/iris.functions";

export const Route = createFileRoute("/app/iris/$threadId")({
  component: IrisThreadPage,
});
...
type IrisRow = { id: string; role: string; parts: unknown; created_at: string };

function IrisThreadPage() {
  const { threadId } = Route.useParams();

  const historyQ = useQuery({
    queryKey: ["iris-messages", threadId],
    queryFn: () => getIrisMessages({ data: { threadId } }),
  });

  const initialMessages = useMemo<UIMessage[]>(() => {
    const rows = (historyQ.data ?? []) as IrisRow[];
    return rows.map((r) => ({
      id: r.id,
  ...
  return <ChatSurface key={threadId} threadId={threadId} initialMessages={initialMessages} />;
}
