...
export const getBrandAnalytics = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const [{ data: campaigns }, { data: deals }] = await Promise.all([
      ...
      supabase.from("deals").select("id, stage, offer, creator_user_id, campaign_id, updated_at, creator:creator_profiles!deals_creator_user_id_fkey(display_name)"),
    ]);
    ...
    // Top creators by deal count / offer
    const byCreator = new Map<string, { name: string; total: number; count: number }>();
    for (const d of deals ?? []) {
      const key = d.creator_user_id;
      const entry = byCreator.get(key) ?? { name: d.creator?.display_name ?? "Creator", total: 0, count: 0 };
